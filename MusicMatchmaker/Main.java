...
public class Main extends Application {
	//Panes for the main GUI
	private BorderPane mainWindowPane = new BorderPane();
	private TopPane topControlBar = new TopPane();
	private CenterPane centerSongPane = new CenterPane();
	private RightPane rightPlaylistPane = new RightPane();
	private LeftPane leftPopoutPane = new LeftPane();
	...
	static String playlistDisplayed = null;
	static boolean breakout = false;
	TextInputDialog playlistCreateDialog;
	...
	public void start(Stage primaryStage) throws Exception {
        ...
        //This creates an instance of MusicPlayer
        MusicPlayer musicPlayer = new MusicPlayer();
        
        //This creates an instance of MediaPlayerUI and passes the MusicPlayer instance to it
        MediaPlayerUI mediaPlayerUI = new MediaPlayerUI(musicPlayer);

        //This Creates a scene and sets up the root node of MediaPlayerUI
        Scene mediaScene = new Scene(mediaPlayerUI.getRoot(), 1000, 500);

		// If the amount of songs isn't divisible by 25 we have to add one extra page to
		// show said songs
		...
		//Variables from RightPane
		ScrollPane playlistScrollPane = rightPlaylistPane.getPlaylistPane();
		GridPane playlistGrid = rightPlaylistPane.getPlaylistGrid();
		...
		playlistScrollPane.widthProperty().addListener((obs, oldVal, newVal) -> {
			playlistGrid.setMinWidth((double) newVal);
		});

		playlistScrollPane.heightProperty().addListener((obs, oldVal, newVal) -> {
			playlistGrid.setMinHeight((double) newVal);
		});

		// Fill song list with songThreshold number of songs
		updateListVariables(Song.songList);
		updateSongList(currSongList, songGridPane, currSong, songThreshold, -1, mediaScene);
		...
		gotoPage.setOnKeyPressed(new EventHandler<KeyEvent>() {
			public void handle(KeyEvent e) {
				if (e.getCode() == KeyCode.ENTER) {
					...
					updateSongList(currSongList, songGridPane, currSong, songThreshold,
							Integer.parseInt(gotoPage.getText()), mediaScene);
					gotoPage.clear();
					gotoPage.setPromptText(String.valueOf(currPage));
		...
		mainWindowPane.setRight(rightPlaylistPane);
		...
		playlistScrollPane.getStyleClass().add("pane");
		playlistGrid.getStyleClass().add("pane");
		...
		RadioButton[] searchTypeButtons = { new RadioButton("Song Name"), new RadioButton("Artist Name"),
				new RadioButton("Genre") };
		...
		topControlBar.getSearchButton().setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent e) {
				...
				case "song":
					updateListVariables(
							recommendationHandler.recommendBySimilarGenre(topControlBar.getSearchFieldText()));
					updateSongList(currSongList, songGridPane, currSong, songThreshold, 1, mediaScene);
					break;
				case "artist":
		...
		playlistCreateDialog = new TextInputDialog("");
		playlistCreateDialog.setHeaderText("Enter a playlist name");
		playlistCreateDialog.setTitle("Playlist Creator");

		rightPlaylistPane.getAddButton().setOnAction(new EventHandler<ActionEvent>() // Open dialog box to add a new playlist
		{
			public void handle(ActionEvent event) {
				playlistCreateDialog.showAndWait();

				String result = playlistCreateDialog.getResult();

				if (result != null && !result.isBlank()) {
					Playlist.createPlaylist(result);
				}
				
				rightPlaylistPane.populateList(null);
			}
		});
	...
	// Main GUI methods
	// Function to update the songGridPane, if gotoPage is not -1 it is invoked by
	// the TextField
	public void updateSongList(List<Song> songList, GridPane songListGrid, int currSongNum, int songThreshold, int gotoPage, Scene mediaScene) {
		...
		songListGrid.add(new Label("Album"), 4, 0);
		songListGrid.add(new Label("Genre"), 5, 0);
		...
		for (int i = currSongNum; i < currSongNum + songThreshold && i < songList.size(); i++) {
			...
			String[] songInfo = { currentSong.getTrackName(), currentSong.getArtistName(), currentSong.getAlbumName(),
			...
			for (int j = 0; j < 4; j++) {
				if (j < 1) {
					...
					songListGrid.add(new Button("", playButton), j,
							(i % 25) + 1);
					playButton.setOnAction(centerSongPane.getPlayButtonHandler(mediaScene, songInfo[0]));
				}

				songListGrid.add(new Label(songInfo[j]), j + 2, (i % 25) + 1);
			}
		}
	}
